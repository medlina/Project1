...
For the last step let's compare results of our Linear and ARIMA models. For that we devided our Data Set into two parts - a training one (75% of all data points), and a test one (25% of all data points).
...
lines(testing$usage ~ testing$day, col = "green", type = 'o')
par(new=TRUE)
plot(model3_forecast, col = color, shaded=TRUE, shadecols = color, fcol = "red")
par(new=F)

```

As we can see from the plots, linear model that was built on smaller ammount of data points is absolytelly different to the previous one. At the same time, ARIMA (2,0,0) model gives acceptable results.  